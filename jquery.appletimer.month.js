(function ($, window, document, undefined) {
	...
	var p = 'appleTimer',
		def = {
			 ...
			 * details.
			 */
			decisecondsWrapper: '.at-dseconds',
	...
	Plugin.prototype = {
		...
		calculate: function() {
			...
			var m = t.round(dTC / dim);
			//alert(t.round(dTC / dim));
			//dTC = dTC * 31;
			//alert(dTC);
			//alert(dTCC);
			...
			var m = t.round(dTC / dim);
			dTCC = dTCC % dim;
			var h = t.round(dTCC / him);
			dTCC = dTCC % him;
			//alert(dTCC);
			var i = t.round(dTCC / iim);
			dTCC = dTCC % iim;
			var se = t.round(dTCC / seim),
				m = dTCC % seim,
				ds = t.round(m / deim);
		...
		format: function() {
			...
			t.monthsLabel = (m == 1 && mmL !== null && sL == true) ? mmL : s.monthsLabel;
		...
		output: function() {
			...
			t.writeLabelToDom(s.daysLabelWrapper, t.monthsLabel);
			...
			t.writeDigitsToDom(s.daysWrapper, t.months, 'at-months-digit');
			t.writeDigitsToDom(s.daysWrapper, t.days, 'at-days-digit');
			t.writeDigitsToDom(s.hoursWrapper, t.hours, 'at-hours-digit');
			t.writeDigitsToDom(s.minutesWrapper, t.minutes, 'at-minutes-digit');
			t.writeDigitsToDom(s.secondsWrapper, t.seconds, 'at-seconds-digit');
			t.writeDigitsToDom(s.decisecondsWrapper, t.deciseconds, 'at-dseconds-digit');
			t.writeDigitsToDom(s.millisecondsWrapper, t.milliseconds, 'at-mseconds-digit');
